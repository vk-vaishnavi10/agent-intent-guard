...
def build_padded_features(pad_repeats):
    rows = []
    for _, r in attack_test.iterrows():
        padded_memory = r["memory"] + ((" " + FILLER * pad_repeats) if pad_repeats else "")
        query_addrs = extract_addrs(r["query"])
        mem_addrs = extract_addrs(padded_memory)
        foreign = mem_addrs - query_addrs
        sim = max_query_similarity(r["query"], padded_memory)
        rows.append({
            "memory_len_capped": min(len(padded_memory), LEN_CAP),
            "injection_keyword_score": injection_score(padded_memory),
            "num_foreign_addrs": len(foreign),
            "has_foreign_addr": int(len(foreign) > 0),
            "query_memory_similarity": sim,
            "starts_with_imperative": starts_with_imperative(padded_memory),
        })
...
for pad in PAD_REPEATS:
    pdf = build_padded_features(pad)
    avg_len = pdf["memory_len_capped"].mean()
    r_lr = recall_score(y_true, lr.predict(pdf[feature_cols].values))
    r_full = recall_score(y_true, xgb_full.predict(pdf[feature_cols].values))
    r_noKw = recall_score(y_true, xgb_noKw.predict(pdf[ablation_cols].values))
    r_rule = (pdf["injection_keyword_score"] > 0).astype(int).mean()
